...
class Sentinel2Raster():
    ...
    def __reproject_bands(self):
        ...
        # Read all designated bands
        for band, file in self.band_files.items():
            extracted_band[band] = rio.open(self.src_zip.open_raster(file))
            if extracted_band[band].res[0] == min_res and ref_raster is None:
                ref_raster = extracted_band[band]
        for band, raster in extracted_band.items():
            if raster.res != ref_raster.res:
                tmp = extracted_band[band]
                # I use nearest neighbor here since it is the fastest and I want to keep numpy array memory footprint
    ...
    def get_band_array(self, band):
        if band == "SCL":
            ...
            # I would keep it as Digital Number since it will be smaller to store int16 than float
            # return (self.band[band].read(1) - self.boa_effect).astype(float) * self.scaling_factor
